...
function initConstructor() {
    ...
    function initEjercicios() {
        ...
        function mostrarResultadoFinal() {
            ...
            const tiempoAgotado = segundosTranscurridos >= TIEMPO_LIMITE;
            const puntos = tiempoAgotado ? 0 : obtenerPuntosFinales(idEjercicioActivo, segundosTranscurridos);
            const datosEj = CATALOGO_ALGORITMOS.find(e => e.id === idEjercicioActivo);
            resultado.innerHTML = `
                <p style="color: white;">¡Reto Terminado!</p>
                <p style="color: white;">Ejercicio: ${datosEj.enunciado}</p>
                <p style="color: white;">Puntos obtenidos: <span style="font-size: 1.5em; color: white;">${puntos}</span></p>
                ${tiempoAgotado ? '<p style="color: #ef4444;">⏰ ¡Se agotó el tiempo!</p>' : ''}
            `;
        }
...
function initRegistro() {
    ...
    function renderPlayers() {
            ...
            html += `
                <tr class="${firstCls}" data-id="${p.id}">
                    ...
                    <td class="puntos" data-label="Puntos">
                        <span class="points-total">${p.points}</span>
                        <input type="number" class="points-add-input" value="" min="0" placeholder="+pts" data-id="${p.id}">
                        <button class="btn-add-points" data-id="${p.id}" title="Sumar puntos">➕</button>
                    </td>
        ...
        // Eventos de sumar puntos con botón +
        container.querySelectorAll('.btn-add-points').forEach(btn => {
            btn.addEventListener('click', e => {
                const id = parseInt(e.currentTarget.dataset.id);
                const input = container.querySelector(`.points-add-input[data-id="${id}"]`);
        ...
        // También sumar al pulsar Enter en el input
        container.querySelectorAll('.points-add-input').forEach(input => {
